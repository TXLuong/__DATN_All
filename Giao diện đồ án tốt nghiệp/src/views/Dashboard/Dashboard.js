...
import EmployeeBox from '../../components/Admin/EmployeeBox'
// thoi gian turn in 
// thoi gian leave
...
export default function WorkLog(){
    ...
    useEffect((() => {
      authGet(dispatch, token, "/employees").then(
        (res) => {
          if(res == null){
            alert("Can not fetch employees");
            return ;
          }
          else if (res.rows == null){
            alert("There isn't any employee");
            return 
          }
          let num_emp = res.rows.length;
          let emps = [];
          let rows = res.rows;
    ...
    const handleSeeHistory = () => {
      ...
      authPost(dispatch, token,"/admin/history",body).then(
        (res) => {
          let rows_res = [];
          let arr = res.rows;
          if(arr == null){
    ...
    return (
      ...
      <div className = {classDate.listDate}>
        <GridContainer>
          ...
          <GridItem xs={12} sm={3} >
            <EmployeeBox employees = {employees} setEmp_id = {setEmp_id}></EmployeeBox>
          </GridItem>
          <GridItem xs={12} sm={3} >
